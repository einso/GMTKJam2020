...
Shader "Shader Forge/TerrainCustom" {
    Properties {
        ...
        _normaltest ("normal test", Range(0, 1)) = 0
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _normaltest)
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_4865 = cross(normalize(ddx(i.posWorld.rgb)),normalize(ddy(i.posWorld.rgb)));
                float _normaltest_var = UNITY_ACCESS_INSTANCED_PROP( Props, _normaltest );
                float3 normalLocal = lerp(i.normalDir,node_4865,_normaltest_var);
